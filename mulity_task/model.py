...
class Model(BaseModel):
    ...
    def __init__(self, is_training, num_classes_dict={'digits': 10, 'letters': 24}):
        """Constructor.
    ...
    """
    多任务多标签任务的 CNN 定义也非常简单，只需要引入一个 for 循环即可。接下来，要定义损失函数和准确率函数。
    """
    def predict(self, preprocessed_inputs):
    ...
    def _onehot_groundtruth_dict(self, groundtruth_lists):
        """
        _onehot_groundtruth_dict 用于将统一分配的类标号对应的独热编码分为数字和字母这两个任务对应的两个独热编码，
        之后的 loss 函数就可以用来计算正常的分类交叉熵损失。为了确保全 0 的独热编码对应 0 的损失，定义了 weights 这一个变量，
        它的作用是：当编码为全 0 时，该样本对应的损失权重为 0，因此贡献的损失为 0，
        即不属于这个分类任务的样本对这个分类任务的损失贡献为 0（虽然理论上全 0 的独热编码对应的分类交叉熵为 0，
        但为了确保这点而不出现意外，weights 是非常必要的
