...
camvidpath = '/Users/miao/Documents/tensorflow/SegNet-Tutorial-master/CamVid/'
# Training data
    ...
    for epoch_i in range(n_epochs):
        ...
        for _ in range(num_batch):
            ...
            if resumeTraining and epoch_i % 10 == 0:
                ...
                plt.title('Input')
                plt.subplot(2, 2, 2);
                plt.pcolor(xs, ys, gtimg, vmin=0, vmax=nrclass);
                plt.title('Ground truth')
                plt.subplot(2, 2, 3);
                plt.pcolor(xs, ys, predMaxOut[0, :, :].reshape(height, width), vmin=0, vmax=nrclass);
                plt.title('[Training] Prediction')
                plt.subplot(2, 2, 4);
                plt.imshow(np.abs(errimg) > 0.5);
                plt.title('Error')
                plt.show()
                # Test data
