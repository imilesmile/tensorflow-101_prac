...
def cae(_X, _W, _b, _keepprob):
    ...
    # decoder
    _cd3 = tf.nn.sigmoid(
        tf.add(tf.nn.conv2d_transpose(_ce3, _W['cd3'], tf.pack([tf.shape(_X)[0], 7, 7, n2]), strides=[1, 2, 2, 1]
                                      , padding='SAME'), _b['bd3']))
...
# define function
x = tf.placeholder(tf.placeholder(tf.float32, [None, dim]))
y = tf.placeholder(tf.placeholder(tf.float32, [None, dim]))
